...
3. **Zone classification maps scores to retention/neutral/distribution**  
   ...
   - Zone boundaries are configurable (e.g., <30 = retention, 30-70 = neutral, >70 = distribution).  
...
- [ ] **Task 1 – Create scoring module structure** (AC: 1, 2)
...
- [ ] **Task 2 – Implement phase score calculation** (AC: 2)
...
- [ ] **Task 3 – Implement zone classification** (AC: 3)
...
- [ ] **Task 4 – Unit tests for scoring logic** (AC: 4)
  ...
  - [ ] Test zone classification at boundaries and within each zone
  - [ ] Verify custom zone thresholds work as expected
  _[Source: docs/architecture.md#Testing Strategy (Pipeline Unit Tests)]_

- [ ] **Task 5 – Integrate scoring into CLI pipeline** (AC: 5)
...
- [ ] **Task 6 – Documentation and comments** (AC: 6)
...
- **Retention Zone** (low phase score): Typically indicates accumulation, low risk of selling pressure.
- **Neutral Zone** (mid-range): Market is balanced, no strong directional signal.
- **Distribution Zone** (high phase score): Indicates elevated selling pressure, higher risk.

Default MVP thresholds:
- Retention: `phase_score < 30`
- Neutral: `30 <= phase_score <= 70`
- Distribution: `phase_score > 70`
...
- **Unit Tests** (`pipeline/tests/unit/`):
  ...
  - Test `classify_zone` with scores at boundaries (29.9, 30.0, 30.1, etc.).
