...
- **Gate Decision:** PASS
- **Scope:** Story 1.5 – Production Scheduling & GitHub Actions Job
- **Summary:**
  - `.github/workflows/pipeline-daily.yml` runs on a daily cron and via manual dispatch, uses Python 3.11 + `uv`, executes the full test suite via `./run-tests`, then runs the `timing-terminal-pipeline` CLI.
  - The workflow asserts that `pipeline/out/chart-data.json` exists and fails the job if it does not.
  - Provider mode is controlled via the `TT_PIPELINE_MODE` configuration (defaulting to `fixture`, overrideable via a GitHub Actions variable) and matches the current `config.py` behavior.
  - An optional Discord notification step sends a simple success/failure message when `DISCORD_WEBHOOK_URL` is configured, without breaking the job on webhook failure.
  - README documents the workflow’s purpose, schedule, behavior, and how to adjust variables/secrets.
- **Testing:** `./run-tests` passes (36 tests: unit + integration) after all Story 1.5 changes.
- **Risks / Notes:** Provider mode currently still uses the in-memory provider abstraction; when a real HTTP provider is introduced, additional secrets and tests should be added in a future story.
