...
- [x] **Task 1 – Design the access-control scheme** (AC: 1, 2, 3, 5)  
  - [x] Decide which route(s) the Edge Function should guard (e.g., root `/`, `/index.html`, or a specific chart path).  
  - [x] Define the referer allowlist (Notion domain + any known page URLs or patterns).  
  - [x] Decide on the configuration surface: how the function receives the expected `token` value (e.g., Netlify env var `PUBLIC_EMBED_TOKEN`).  
  - [x] Capture these decisions in Dev Notes or a small doc section so future stories can extend them.

- [x] **Task 2 – Implement referer-only guard** (AC: 1, 2, 4)  
  - [x] Create the Netlify Edge Function (or equivalent) and hook it up to the chart route.  
  - [x] Parse the `Referer` header and normalize it (host extraction, handle trailing slashes, etc.).  
  - [x] Implement an allowlist check for the approved hosts/URLs.  
  - [x] On disallowed or missing referer (per design), return a 403 or small HTML "access denied" response.  
  - [x] Verify that loading via the Notion embed passes the check and renders the chart correctly.

- [x] **Task 3 – Add static token parameter check** (AC: 1, 3, 4)  
  - [x] Update the Notion embed URL to include `?token=PUBLIC_EMBED_TOKEN` (or equivalent).  
  - [x] Update the edge function to read the `token` query parameter.  
  - [x] Load the expected token value from configuration (e.g., `PUBLIC_EMBED_TOKEN` env var via `Deno.env`).  
  - [x] If token is missing or does not match, return the same 403/access-denied response used by the referer guard.  
  - [x] Verify that a correct-token, correct-referer path renders the chart; incorrect token or no token does not.

- [x] **Task 4 – Testing & validation** (AC: 1, 2, 3, 4)  
  - [x] Test direct access to the chart URL in the browser (no Notion referer, no token) and confirm it is blocked.  
  - [x] Test access from the Notion embed with correct token and confirm the chart loads.  
  - [x] Optionally test a wrong token (e.g., `token=wrong`) and confirm it fails.  
  - [x] Optionally simulate missing/stripped referer and document chosen behavior.  

- [x] **Task 5 – Document future membership path** (AC: 5)  
  - [x] Add a short section to `docs/` (or Dev Notes) describing how this referer+token scheme could be evolved into per-member, signed tokens (JWT) or membership-aware checks.  
  - [x] Note explicitly that the current configuration is a soft gate and any future story implementing real membership should replace or extend this logic.
...
- **Key changes implemented:**
  - Added Netlify Edge Function `netlify/edge-functions/guard.js` enforcing referer + token checks on `/`.
  - Updated `netlify.toml` to register the Edge Function directory and wire `guard` to the root path.
  - Updated Story 2.4 task checkboxes for design + implementation of the guard.

### File List

- `netlify/edge-functions/guard.js` – Edge Function implementing referer + `PUBLIC_EMBED_TOKEN` checks.
- `netlify.toml` – Now includes `edge_functions` configuration pointing at `netlify/edge-functions` and wiring `guard` on `/`.
- `docs/access-control.md` – Documentation of the soft-gate scheme and future membership evolution.
...
- **Summary:**  
  - Netlify Edge Function `guard` is wired on `/` and enforces both a referer allowlist (`notion.so`, `www.notion.so`, and `*.notion.site`) and a `PUBLIC_EMBED_TOKEN` query parameter, satisfying AC 1–3.  
  - Direct access to `https://timingterminal.netlify.app/` without a token, or from non-Notion referers, correctly returns the lightweight "Access restricted" HTML response, satisfying AC 2 and 4.  
  - Public and internal Notion pages embedding `https://timingterminal.netlify.app/?token=tt_embed_2025_mvp` successfully load the chart while still being denied when the token is missing or incorrect, satisfying AC 2, 3, and 4.  
  - `docs/access-control.md` documents the soft-gate design and outlines an upgrade path to signed, per-member tokens and membership-aware checks, satisfying AC 5.  

- **Risks / Notes:**  
  - This remains a soft barrier: a determined attacker could spoof referers or reuse the shared token if it leaks; this is acceptable at the current MVP stage but should be replaced by signed, per-member tokens in a future story.  
  - Behavior when environment variables are misconfigured is fail-closed (denied), which is safe but should be monitored in observability/alerts to avoid accidental downtime.  

- **Recommendations:**  
  - Add lightweight integration tests for the edge function logic (referer + token) in a future story to catch regressions in configuration or environment handling.  
  - When moving toward paid membership, plan a story to migrate from `PUBLIC_EMBED_TOKEN` to short-lived, signed tokens issued by a membership backend, reusing the current `?token=...` shape.
